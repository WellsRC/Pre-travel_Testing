...
for vv=1:3
    ...
    % Infected after the test was adminsted 
        ...
        PretestIS(jj)=((1./(ts-w(jj))).*integral2(@(u,t)InfectiousnessfromInfectionTesting(t,u,[],testtype,R0S,R0A,0,ts,td,SelfIsolate,betaRTPCR,mm,PCR_Map),0,ts-w(jj),@(u)(u+w(jj)),ts));
        PretestIA(jj)=((1./(td-w(jj))).*integral2(@(u,t)InfectiousnessfromInfectionTesting(t,u,[],testtype,R0S,R0A,1,ts,td,0,betaRTPCR,mm,PCR_Map),0,td-w(jj),@(u)(u+w(jj)),td));

    end

    save(['Pre_Testing_No_Testing_' VOC{vv} '.mat']);
end