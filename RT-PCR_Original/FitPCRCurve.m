...
betat=log(Gx(1).*((Gx(2)-1)/Gx(2))^(1/Gx(2)))+beta(1)^2;
beta=[betat beta];
save('MLE-Estimate-RTPCR_Original.mat');
...
b1=10.^b1;
Pk=10.^Pk;
betaRTU=[log(Gx(1).*((Gx(2)-1)/Gx(2))^(1/Gx(2)))+b1.^2 b1 Pk];
betaRTU=betaRTU(~isinf(L),:);
L=L(~isinf(L));
save('Uncertainty-BetaEstimate-RTPCR_Original.mat');