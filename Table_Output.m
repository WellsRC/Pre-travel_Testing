...
for vv=1:3
   ...
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    Name_Red=cell(NumTests+1,5);
    Name_PT=cell(NumTests+1,6);
    for jj=1:NumTests+1
           switch  jj
               case 1
                    Name_Red{jj}='RT-PCR';
           ...
           for ww=1:5
               Time_Test_PT{jj,ww+1}=[num2str(MLE_PT(jj,ww),'%4.2f') '% (' num2str(LB_95_PT(jj,ww),'%4.2f') '%' char(8211) num2str(UB_95_PT(jj,ww),'%4.2f') '%)'];               
               Time_Test_Red{jj,ww}=[num2str(100.*MLE_Red(jj,ww),'%4.2f') '% (' num2str(100.*LB_95_Red(jj,ww),'%4.2f') '%' char(8211) num2str(100.*UB_95_Red(jj,ww),'%4.2f') '%)'];               
           end
    end
     
    writetable(table(Name_Red,Time_Test_Red),[VOCv{vv} '_Reduction_Transmission.csv']);
    writetable(table(Name_PT,Time_Test_PT),[VOCv{vv} '_Reduction_Transmission.csv']);  
end
